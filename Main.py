...
def write_to_db_afd_demo(url, page_title, word):
    conn = sqlite3.connect("PoliticalPartyWebScraper.db")
    c = conn.cursor()
    c.execute("INSERT INTO democracy_words_from_afd VALUES (?,?,?)", (url, page_title, word))
    conn.commit()


def write_to_db_spd_demo(url, page_title, word):
    conn = sqlite3.connect("PoliticalPartyWebScraper.db")
    c = conn.cursor()
    c.execute("INSERT INTO democracy_words_from_spd VALUES (?,?,?)", (url, page_title, word))
    conn.commit()
    conn.close()


def write_to_db_cdu_demo(url, page_title, word):
    conn = sqlite3.connect("PoliticalPartyWebScraper.db")
    c = conn.cursor()
    c.execute("INSERT INTO democracy_words_from_cdu VALUES (?,?,?)", (url, page_title, word))
    conn.commit()
    conn.close()


# function to write immigration-related words to database
def write_to_db_afd_immigrate(url, page_title, word):
    conn = sqlite3.connect("PoliticalPartyWebScraper.db")
    c = conn.cursor()
    c.execute("INSERT INTO immigration_words_from_afd VALUES (?,?,?)", (url, page_title, word))
    conn.commit()
'''
# web scrape AfD
print("Scraping AfD...\n")
...
def scrape_afd():
    ...
    try:
        ...
        for word in upper_words_split:
            word = word.strip(r".,<>'/\|_=?!")
            if word in democracy_key_words:
                afd_word_list.append(word)
                write_to_db_afd_demo(str(page_url), str(page_title), str(word))
            if word in populism_key_words:
                afd_word_list.append(word)
                write_to_db_afd_pop(str(page_url), str(page_title), str(word))
            elif word in immigration_key_words:
                afd_word_list.append(word)
                write_to_db_afd_immigrate(str(page_url), str(page_title), str(word))
    ...
    for element in page_text:
        ...
        for word in words:
            ...
            if word in democracy_key_words:
                ...
                write_to_db_spd_demo(str(spd_url), str(spd_page_title), str(word))
            if word in populism_key_words:
                spd_word_list.append(word)
...
def scrape_cdu():
    ...
    for word in words:
        ...
        if word in democracy_key_words:
            ...
            write_to_db_cdu_demo(str(cdu_url), str(cdu_page_title), str(word))
        if word in populism_key_words:
            cdu_word_list.append(word)
...
cdu_pages = ["Arbeit und Soziales", "Außen- und Verteidigungspolitik", "Bildung und Forschung", "Demografie",
             ...
             "Innere Sicherheit und Rechtspolitik", "Integration, Zuwanderung und Aussiedler", "Kommunalpolitik, Ehrenamt und Sport", "Kultur, Medien und Netzpolitik", "Landwirtschaft und ländlicher Raum", "Menschen mit Behinderungen/Inklusion", "Umwelt, Natur und Klimaschutz", "Verbraucherschutz", "Verkehr, Bau und Infrastruktur", "Wirtschaft"]
cdu_page_num = 0
